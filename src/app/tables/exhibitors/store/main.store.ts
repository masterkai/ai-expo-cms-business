...
export const MainStore = signalStore(
	...
	withComputed((store) => {
		...
		const computed_sponsor_ships = computed(() =>
	...
	withMethods((store) => {
		const setExhibitionRights = (rights: Exhibition_rights) =>
			patchState(store, updaters.setExhibitionRights(rights));
		...
		const setSelectedSponsorShips = (option: GenericOption | null) =>
			patchState(store, updaters.setSelectedSponsorShips(option));
		...
		const _companyDATAQuery = runInInjectionContext(store._injector, () =>
			injectQuery(() => ({
		...
		const setCurrentCompID = (compID: string | null) =>
				...
				next: (response) => {
					if (response.status === 'success' && response.data) {
						const data: Exhibition_rights = {
							rights_level: response.data.rights_level,
		...
		const onSaveExhibitorRights = () => {
			const compID = store.isRightChangeModeEnabled()
				? store._rightChangeStore.current_compID()
			...
			const successMessage = store.isRightChangeModeEnabled()
				? '展覽權益變更需求已送出。'
				: '展覽權益資料已更新。';
			const errorMessage = store.isRightChangeModeEnabled()
				? '送出展覽權益變更需求失敗，請稍後再試。'
				: '更新展覽權益資料失敗，請稍後再試。';
			const rights_level =
				store.selectedSponsorShips() !== null
					? store.selectedSponsorShips()!.name
					: (store.computed_sponsor_ships() ?? '');
			store._exhibitionRightsService
				.setRights({
					compID: compID ?? '',
					rights_level,
					type,
		...
		const setGridNumber = (gridNumber: string) =>
			patchState(store, updaters.setGridNumber(gridNumber));

		const setSelectedExhibitionRights = (
			partialSelectedRights: Partial<Selected_Exhibition_rights>,
		) => patchState(store, updaters.setSelectedExhibitionRights(partialSelectedRights));

		const setIsDialogVisible = (visible: boolean) =>
			patchState(store, updaters.setIsDialogVisible(visible));

		const setCurrentCompany = (company: Company | null) =>
			patchState(store, updaters.setCurrentCompany(company));
		...
		const setSelectedBoothStyle = (style: BoothStyles | null) =>
			patchState(store, updaters.setSelectedBoothStyle(style));

		const setGridNumUI = (v: number | null) => patchState(store, updaters.setGridNumUI(v));

		const setCurrentReviewDATA = (data: CompanyUpdatePayload[]) =>
			patchState(store, updaters.setCurrentReviewDATA(data));
		...
		return {
			...
			onSetExhibitionLink,
			resetSelectedExhibitionRights: () =>
				patchState(store, updaters.resetSelectedExhibitionRights()),
			onGetLink,
			setEmpno,
			setDepartID,
			setBoothStyle,
						...
						const mapSelected = (items: Option[] | undefined, options: Option[] | undefined) => {
							...
							const mapped = items
								.map((i) =>
									options.find(
										(o) =>
											String(o.id) === String((i as any).id) ||
											String(o.option).trim() === String((i as any).option).trim(),
									),
								)
